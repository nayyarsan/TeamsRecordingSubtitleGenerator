...
class VideoVisualizer:
        ...
        try:
            while True:
                ...
                if frame_count in frame_data_map:
                    ...
                    current_speaker = self._get_speaker_at_time(
                        current_time, speaker_timeline
                    )
                    current_text = self._get_transcript_at_time(
                        current_time, transcript_timeline
                    )
                    ...
                    frame_with_subtitle = self._draw_subtitle(
                        frame_with_subtitle, current_speaker, current_text, height
                    )
                else:
    ...
    def _add_subtitle_space(
        self, frame: np.ndarray, subtitle_height: int
    ) -> np.ndarray:
    ...
    def _draw_face_box(self, frame: np.ndarray, face: Face) -> np.ndarray:
    ...
    def _draw_subtitle(
        self, frame: np.ndarray, speaker: str, text: str, original_height: int
    ) -> np.ndarray:
    ...
    def _build_speaker_timeline(
        self, fused_segments: List[SpeakerSegment], speaker_mapping: Dict[str, str]
    ) -> List[Tuple[float, float, str]]:
        """Build timeline of (start, end, speaker_name) tuples."""
        timeline = []
        for segment in fused_segments:
            speaker_name = speaker_mapping.get(
                segment.speaker_cluster_id, f"Speaker {segment.speaker_cluster_id}"
            )
    ...
    def _get_speaker_at_time(
        self, current_time: float, speaker_timeline: List[Tuple[float, float, str]]
    ) -> str:
    ...
    def _get_transcript_at_time(
        self, current_time: float, transcript_timeline: List[Tuple[float, float, str]]
    ) -> str:
    ...
    def _finalize_video(
        self, temp_path: Path, output_path: Path, ffmpeg_path: Optional[str] = None
    ):

...
class AudioProcessor:
    ...
    def __init__(
        self, config: Optional[Dict] = None, ffmpeg_path: Optional[str] = None
    ):
        """
    ...
    def extract_audio(
        ...
        if output_path is None:
            output_path = (
                video_path.parent / f"{video_path.stem}_audio.{self.audio_format}"
            )
    ...
    def load_diarization_pipeline(self):
        ...
        if self._diarization_pipeline is None:
            ...
            try:
                ...
                if not token:
                    logger.warning(
                        "HF_TOKEN not set. Attempting to load without token (may fail for gated models)."
                    )
                ...
                logger.info(
                    "1. Visit and accept: https://huggingface.co/pyannote/speaker-diarization-3.1"
                )
                logger.info(
    ...
    def perform_diarization(self, audio_path: Path) -> List[DiarizationSegment]:
        """
        ...
        try:
            ...
            logger.info(
                f"Running diarization (audio duration: {waveform.shape[1] / 16000:.1f}s)"
            )
            ...
            logger.info(
                f"Diarization complete: {len(segments)} segments from {len(set([s.speaker_id for s in segments]))} speakers"
            )
            return segments
        except Exception as e:

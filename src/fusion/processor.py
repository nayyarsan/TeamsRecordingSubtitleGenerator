...
class AudioVisualFusion:
    ...
    def fuse(
        ...
        """
        logger.info(f"Fusing {len(diarization_segments)} audio segments with "
        ...
        for diar_seg in diarization_segments:
            ...
            overlapping_frames = self._get_overlapping_frames(
                diar_seg, frame_data_list
            )
            ...
            face_id, confidence = self._identify_speaking_face(
                overlapping_frames
            )
        ...
        logger.info(f"Created {len(fused_segments)} fused segments, "
    ...
    def _get_overlapping_frames(
        ...
        for frame_data in frame_data_list:
            ...
            if (segment.start - self.alignment_tolerance <= 
    ...
    def build_speaker_face_mapping(
        ...
        for segment in fused_segments:
            if segment.face_id:
                ...
                weight = (
                    segment.confidence_scores['av_alignment'] * 
                    segment.duration
                )
    ...
    def get_statistics(
        ...
        stats = {
            ...
            'avg_confidence': {
                'diarization': np.mean([s.confidence_scores['diarization'] 
                                       for s in fused_segments]),
                'av_alignment': np.mean([s.confidence_scores['av_alignment'] 
                                        for s in fused_segments]),
            },

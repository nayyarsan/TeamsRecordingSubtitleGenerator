...
class VideoProcessor:
    ...
    def process_video(
        ...
        logger.info(f"Video: {original_fps:.2f} FPS, {total_frames} frames, {duration:.2f}s")
        ...
        logger.info(f"Video processing complete: {len(frame_data_list)} frames processed, "
    ...
    def _track_faces(
        ...
        active_tracks = {
            fid: track for fid, track in self.face_tracks.items()
    ...
    def _detect_lip_movement(
        ...
        if results.multi_face_landmarks:
            ...
            for idx, face in enumerate(faces):
                ...
                history = [
                    f.lip_movement 
                    for ts, f in self.face_history[face.face_id][-self.lip_window_size:]

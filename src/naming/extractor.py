...
class SpeakerNamer:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.intro_patterns = intro_config.get('intro_patterns', [
    ...
    def extract_names(
        ...
        for idx, speaker_id in enumerate(sorted(missing_speakers)):
            ...
            named_speakers.append(NamedSpeaker(
        ...
        logger.info(f"Named {len(named_speakers)} speakers: "
            ...
            for pattern in patterns:
                ...
                for match in matches:
                    ...
                    if self._is_valid_name(name):
                        ...
                        logger.debug(f"Found name candidate: '{name}' in '{segment.text[:50]}...'")
    ...
    def _is_valid_name(self, name: str) -> bool:
        ...
        common_words = {
            'Today', 'Yesterday', 'Tomorrow', 'Everyone', 'Everybody',
            'Please', 'Thank', 'Thanks', 'Welcome', 'Good', 'Morning',
            'Afternoon', 'Evening', 'Meeting', 'Team', 'All'
        }
    ...
    def _map_names_to_clusters(
        ...
        for name, confidence, original_text in name_candidates:
            ...
            speaker_id = self._find_speaker_for_intro(
                intro_seg, fused_segments
            )
    ...
    def _find_speaker_for_intro(
        ...
        for seg in fused_segments:
            ...
            if not (seg.end < intro_segment.start or 
                   seg.start > intro_segment.end):
        ...
        best_speaker = max(
            overlapping,
            key=lambda s: min(s.end, intro_segment.end) - max(s.start, intro_segment.start)
        )
    ...
    def create_speaker_mapping(
        self,
        named_speakers: List[NamedSpeaker]
    ) -> Dict[str, str]:
        """
        ...
        """
        return {
            ns.speaker_cluster_id: ns.name
            for ns in named_speakers
        }
